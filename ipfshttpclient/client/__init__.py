...
DEFAULT_ADDR = multiaddr.Multiaddr(
    os.environ.get("PY_IPFS_HTTP_CLIENT_DEFAULT_ADDR", "/dns/localhost/tcp/5001/http")
)
DEFAULT_BASE = str(os.environ.get("PY_IPFS_HTTP_CLIENT_DEFAULT_BASE", "api/v0"))
DEFAULT_USERNAME: ty.Optional[str] = os.getenv(
    "PY_IPFS_HTTP_CLIENT_DEFAULT_USERNAME", None
)
DEFAULT_PASSWORD: ty.Optional[str] = os.getenv(
    "PY_IPFS_HTTP_CLIENT_DEFAULT_PASSWORD", None
)
...
def assert_version(
    version: str,
    minimum: str = VERSION_MINIMUM,
    maximum: str = VERSION_MAXIMUM,
    blacklist: ty.Iterable[str] = VERSION_BLACKLIST,
) -> None:
...
class Client(files.Base, miscellaneous.Base):
    ...
    __doc__ += "\n\n" + "\n".join(
        l[1:] for l in base.ClientBase.__init__.__doc__.split("\n")
    )
