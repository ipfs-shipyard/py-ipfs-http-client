...
response_item_t = ty.Union[json_primitive_t, "ResponseBase", "_response_item_list_t"]
...
class ResponseBase(ty.Mapping[str, response_item_t]):
    ...
    def __repr__(self) -> str:
        ...
        if attr_str_parts and attr_json_parts:
            arg_str = "{0}, **{{{1}}}".format(
                ", ".join(attr_str_parts), ", ".join(attr_json_parts)
            )
        elif attr_str_parts:
...
class ResponseWrapIterator(ty.Generic[T, R]):
    ...
    def __init__(
        self, inner: http.StreamDecodeIteratorSync[T], item_wrap_cb: wrap_cb_t[T, R]
    ):
        self._inner = inner
...
class _returns_multiple_wrapper2_t(ty_ext.Protocol, ty.Generic[T, R]):
    @ty.overload
    def __call__(
        self, *args: ty.Any, stream: utils.Literal_True, **kwargs: ty.Any
    ) -> ty.List[R]:
        ...

    @ty.overload
    def __call__(
        self, *args: ty.Any, stream: utils.Literal_False = ..., **kwargs: ty.Any
...
def returns_multiple_items(
    item_wrap_cb: wrap_cb_t[T, R] = ident, *, stream: bool = False
...
def returns_single_item(
    ...
    def wrapper1(func: _inner_func_t[T]) -> _returns_single_wrapper2_t[T, R]:
        ...
        def wrapper2(
            *args: ty.Any, **kwargs: ty.Any
        ) -> ty.Union[R, ResponseWrapIterator[T, R]]:
            result = func(*args, **kwargs)
            if isinstance(result, list):
                assert len(result) == 1, (
                    "Called IPFS HTTP-Client function should "
    ...
    def wrapper(
        *args: ty.Any, **kwargs: ty.Any
        ...
        if isinstance(result, (list, bytes, object)):
            assert not result, (
                "Called IPFS HTTP-Client function should never "
...
class SectionProperty(ty.Generic[S]):
    ...
    def __get__(
        self,
        client_object: ty.Optional["ClientBase"],
