...
def content_disposition_headers(
    filename: str, disptype: str = 'form-data; name="file"'
) -> ty.Dict[str, str]:
    ...
    """
    disp = '{0}; filename="{1}"'.format(disptype, urllib.parse.quote(filename, safe=""))
    return {"Content-Disposition": disp}


def content_type_headers(
    filename: str, content_type: ty.Optional[str] = None
) -> ty.Dict[str, str]:
...
def multipart_content_type_headers(
    boundary: str, subtype: str = "mixed"
) -> ty.Dict[str, str]:
...
class _StreamFileMixinProto(ty_ext.Protocol):
    ...
    def _gen_file_chunks(
        self: "_StreamFileMixinProto", file: ty.IO[bytes]
    ) -> gen_bytes_t:
        ...
    ...
    def _body(self) -> gen_bytes_t:
        ...
        try:
            for item in self.scanner:
                ...
                short_path = self.name + (
                    ("/" + relpath_unicode) if relpath_unicode != "." else ""
                )
                ...
                if item.type is filescanner.FSNodeType.FILE:
                    try:
                        ...
                        if item.parentfd is not None:
                            stat_data = os.stat(
                                item.name,
                                dir_fd=item.parentfd,
                                follow_symlinks=self.follow_symlinks,
                            )
                        else:
                            stat_data = os.stat(
                                item.path, follow_symlinks=self.follow_symlinks
                            )
                        ...
                        if self.abspath is not None:
                            absolute_path = os.fsdecode(
                                os.path.join(self.abspath, item.relpath)
                            )
                    ...
                    yield from self._gen_file(
                        short_path, content_type="application/x-directory"
                    )
...
class BytesFileStream(FilesStream):
    ...
    def __init__(
        self,
        data: ty.Union[bytes, gen_bytes_t],
        name: str = "bytes",
        *,
        chunk_size: int = default_chunk_size
    ) -> None:
...
def stream_filesystem_node(
    ...
    *,
    ...
    if is_dir:
        assert not isinstance(filepaths, collections.abc.Iterable) or isinstance(
            filepaths, (str, bytes)
        )
...
def stream_text(
    text: ty.Union[str, ty.Iterable[str]], *, chunk_size: int = default_chunk_size
) -> ty.Tuple[gen_bytes_t, ty.Dict[str, str]]:
